...
for r in repeats:
    for p in protocols:
        ...
        try:
            ...
            print(r)
            for i in range(0, lambdanr):
                folder = '%i/lambda_%i/Production_MD'%(r, i)
                xvg = '%s/prod.xvg' %(folder)
                u_nk = extract_u_nk(xvg, T=298.15)
                print(len(u_nk))
            ...
            print(dGs)
            print(dG_error)
            file = open('%i/dg.pickle'%(r), 'wb')
            pickle.dump(dGs, file)
            file.close()
            file = open('%i/dg_error.pickle'%(r), 'wb')
